...
public class RCC_MobileButtons : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Обновляем состояние NOS кнопки.
        canUseNos = playerCarController.useNOS;

        // Передаём ввод только машине игрока.
        if (playerCarController.canControl && !playerCarController.externalController)
        {
            playerCarController.gasInput = gasInput;
            playerCarController.brakeInput = brakeInput;
            playerCarController.steerInput = -leftInput + rightInput + steeringWheelInput + gyroInput + joystickInput;
            playerCarController.handbrakeInput = handbrakeInput;
            playerCarController.boostInput = NOSInput;
        }
    }
