...
public class RCC_UIDashboardDisplay : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (playerCarController == null)
            return; // Если у объекта нет RCC_CarControllerV3, ничего не делаем.
        ...
        if (heatIndicator)
            heatIndicator.color = playerCarController.engineHeat >= 100f ? Color.red : new Color(.1f, 0f, 0f);

        if (fuelIndicator)
            fuelIndicator.color = playerCarController.fuelTank < 10f ? Color.red : new Color(.1f, 0f, 0f);

        if (rpmIndicator)
            rpmIndicator.color = playerCarController.engineRPM >= playerCarController.maxEngineRPM - 500f ? Color.red : new Color(.1f, 0f, 0f);
